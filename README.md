...
- **BLE T&H repeaters** — devices running [PVVX BLETHR](https://github.com/pvvx/BLETHR) are recognised for what they are: not thermometers, but relays that show and rebroadcast another device's reading. They get their own view and editor, led by the thing that matters — which thermometer each one follows, shown as a name and a signal rather than a second MAC to look up. You can repoint a repeater from a list of the devices already on screen, or type an address for one that is somewhere else entirely, and its source bind key is filled in from that device's own snapshot.
...
- **Snapshots stay with their own kind of device.** A thermometer snapshot and a repeater snapshot describe different hardware, so neither can be restored onto the other — the attempt is refused rather than writing bytes that mean nothing there. This matters on a device that has been reflashed from one to the other, where both kinds of snapshot sit in the same history.
...
If any of these sound familiar, consider [buying me a coffee](https://buymeacoffee.com/Csontikka) or [sponsoring me on GitHub](https://github.com/sponsors/Csontikka) ☕:
